...
const f3 = () => {
  ...
  return a3.indexOf(+i3) != -1
    ? (out3.innerText = 'true')
    : (out3.innerText = 'false')
}
...
const f4 = (arr, elem) => {
  ...
  out4.innerText = arr.indexOf(elem)
}
...
const f5 = () => {
  ...
  out5.innerText = a5.indexOf(+i51, +i52)
}
...
const f7 = (arr, elem) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    if (arr[i] == elem) {
      return (out7.innerText = i)
    }
  }
  return (out7.innerText = -1)
}
...
const f9 = () => {
  ...
  a9.forEach((el) => {
    if (el.indexOf(i9) !== -1) {
      res.push(el.indexOf(i9))
    }
  })
  document.querySelector('.out-9').innerHTML = res.join(' ')
}
