...
function f6() {
  const i1 = document.querySelector('.i-61').value
  const i2 = document.querySelector('.i-62').value
  let out = ''
  if (i1 && i2) a6[i1] = i2
  for (const key in a6) {
    out += `${key} ${a6[key]} <br>`
...
function f7() {
  const i = document.querySelector('.i-7').value
  document.querySelector('.out-7').textContent = i in a7 ? 1 : 0
}
...
function f8() {
  ...
  document.querySelector('.out-8').textContent = i in a7 ? a7[i] : 0
}
...
function f12() {
  let i = document.querySelector('.i-12').value

  for (const key in a12) {
    if (a12[key] == i) delete a12[key]
  }
  f5(a12, '.out-12')
}
...
function f13() {
  ...
  for (const key in a13) {
    if (typeof a13[key] == 'number') count += a13[key]
  }
  document.querySelector('.out-13').textContent = count
}
...
function f14() {
  for (const key in a14) {
    document.querySelector('.out-14').textContent += a14[key][0] + ' '
  }
}
...
function f18() {
  let i = document.querySelector('.i-18').value
  document.querySelector('.out-18').textContent = a18[i] ? a18[i].join(' ') : ''
}
...
function f19() {
  let i = document.querySelector('.i-19').value
  let val = i[0].toUpperCase() + i.slice(1).toLowerCase()
  let out = ''
  for (const key in a19) {
    if (a19[key].includes(val)) out = key
  }
...
function f20() {
  ...
  for (const key in a20) {
    a20[key].forEach((el) => (el[1] == 2 ? (out += el[0] + ' ') : false))
  }
