...
function willYouMarryMe(isPositiveAnswer) {
  return new Promise(((resolve, reject) => {
    if (isPositiveAnswer === true && typeof isPositiveAnswer === 'boolean') {
      resolve('Hooray!!! She said "Yes"!');
    } else if (isPositiveAnswer === false && typeof isPositiveAnswer === 'boolean') {
...
async function chainPromises(array, action) {
  ...
  await array.forEach((el) => {
    el.then((data) => a.push(data)).catch((e) => new Error(e));
  });
  return new Promise((resolve) => resolve(a.reduce(action)));
}
