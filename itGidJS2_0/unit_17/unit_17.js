...
function t4() {
  a4.forEach((el) => (typeof el == 'number' ? a4_res.push(el) : false))
  return a4_res
}
...
function t6() {
  b6_res = b6.filter((el) => typeof el === 'number')
  return b6_res
}
...
function t7() {
  b7_res = b7.filter((el) => el.length > 3)
  return b7_res
}
...
function t8() {
  b8_res = b8.filter((el, ind) => ind % 2 == 0)
  return b8_res
}
...
function t10() {
  b10_res = b10.filter((el) => el.includes(3))
  return b10_res
}
