...
const f6 = () => {
  document.querySelector('.out-6').textContent = new Set(a6).size
}
...
const f7 = () => {
  ...
  document.querySelector('.out-7').textContent =
    i.length == new Set(i).size && i.length > 6 ? 1 : 0
}
...
const f10 = (out_set, elem) => {
  document.querySelector(elem).textContent = [...out_set].join(' ')
}
...
const f13 = () => {
  let set = new Set(str13.split(''))
  let obj = {}
  for (const iter of set) {
    obj[iter] = str13.split('').filter((el) => el == iter).length
  }
