...
function f12() {
  ...
  for (let i = 0; i < a12.length; i++) {
    ...
    for (let j = 0; j < arr.length; j++) {
      if (arr[j] === 1)
        document.querySelector('.out-12').textContent += arr[j] + ' '
    }
  }
}
...
function f13() {
  for (let i = 0; i < 8; i++) {
    a13.push([])
    for (let j = 0; j < 8; j++) {
      a13[i].push(Math.round(Math.random()))
    }
  }
...
function f14() {
  a14.forEach(
    (el) => (document.querySelector('.out-14').textContent += el.length + ' '),
  )
}
...
function f15() {
  let length = 0
  a15.forEach((el) => (el.length > length ? (length = el.length) : false))
  document.querySelector('.out-15').textContent = length
}
